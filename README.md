...
- tableModel.R
    ...
    - Столбцы: vehicle_id(id марки мотоцикла), count_sold(количество проданных товаров которые подходят данной марке), count_publish(количество выставленных товаров которые подходят данной марке), id(id строки), vehicles(название марки мотоцикла)
    - Возвращает: JSON таблицу с задаными елементами
    - начальная дата 2015-06-01
...
- tablePublishTime.R
    ...
    - Принимает: ничего
    - Возвращает: JSON матрицы 24*7 с количеством выставлений на каждый час
- soldProduct.R
    - Имя отчета: таблица количества продаж каждого из товаров
    - Принимает: бренд, номер категории, начальная дата, конечная дата
    - Возвращает: JSON таблицы ProductID, count_sold