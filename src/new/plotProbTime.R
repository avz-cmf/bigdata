...
probTime <- function(brand,CategoryID,begDate,endDate)
{
  ...
  if(checkTable(data.publish) & checkTable(data.sold))
  {
    ...
    plotProbTime <- function(publish = data.publish,
                             sold = data.sold){
      ...
      sold_time_count = hist(as.numeric(format(strptime(sold_time$add_date, FormatDate), "%H")),
                  breaks = seq(-1,23,1),
                  plot = F)$counts;
      
      publish_time = as.numeric(format(strptime(publish$add_date, FormatDate), "%H"));
      
      publish_time_count = hist(publish_time,
    ...
    return(plotProbTime());# вызов функции построения гистограммы
